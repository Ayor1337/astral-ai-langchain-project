...
| `JWT_SECRET_KEY` | 本地账号系统用于签发与校验 JWT 的密钥 |
| `JWT_EXPIRE_SECONDS` | Access token 有效期，默认 `604800` |
| `JWT_ALGORITHM` | JWT 算法，当前固定为 `HS256` |
...
除 `/`、`/docs`、`/redoc`、`/openapi.json`、`/api/auth/register`、`/api/auth/login` 外，其余业务接口都需要带：
