...
Follow PEP 8 with 4-space indentation and explicit imports. Use `snake_case` for functions, modules, and variables; use `PascalCase` for classes such as `Settings` and schema types. Generated code must include concise comments that explain the intent of key logic, especially for non-obvious branches or flows. Every newly added method and every modified method must include an up-to-date docstring. Keep route handlers thin and move provider-specific logic into `app/llm/`. Prefer small, pure helpers over defensive abstraction layers; this repository is intentionally simple.
